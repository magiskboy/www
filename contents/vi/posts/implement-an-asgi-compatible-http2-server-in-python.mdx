...
Hàm `create_server` là một utility function dùng để tạo một socket lắng nghe cho server. Tham số [`reuse_port`](https://man7.org/linux/man-pages/man7/socket.7.html) là một flag cho phép nhiều socket cùng *bind* vào một cổng.  

Để hiểu rõ hơn về cách hoạt động của cơ chế này, hãy cùng xem câu lệnh ngay sau đó.

`os.set_inheritable(self.sock.fileno(), True)` cho phép socket này được kế thừa bởi các *process* con. Nhờ cờ `reuse_port`, các bản sao của socket này có thể *bind* vào cùng một cổng.  
