...
When people solved the ["c10k problem"](http://www.kegel.com/c10k.html) and then the advent of Nginx, asynchronous programming was more interested and improved to make it easier to install than its original version.
...
Now, using asynchronous in Python to be a peace of cake because of the syntactic improvement. The `async/await` syntax was [first introduced by C#](https://en.wikipedia.org/wiki/Async/await) and then implemented by many other programming languages like Javascript, Python, Java, C++, Rust, Swift,...

Asynchronous "programmable technique" switches to "the syntence of a programming language".

## 2. Asynchrony before PEP 492

Asynchronous in Python is completely possible because Python supports non-blocking socket APIs of OS such as selector, multiplexing,... However, using these APIs to deploy an asynchronous application is quite difficult.
...
As a result, programmers have combined those APIs with coroutine to perform asynchronous implementations.
...
If you want to better understand how to use coroutine to implement asynchronous programming, you can read your 2 articles below for more detail:
...
One of the famous frameworks that implements asynchronous is [Tornado](https://en.wikipedia.org/wiki/Tornado_(web_server)). 
...
In 2015, PEP 492 was born proposed by Yury Selivanov. By using coroutine to implement the syntax of async/await, Python has been able to make asynchronous operators much easier for programmers.

Instead of having to work with low-level APIs, working with pure coroutine, now, we only need to know two extremely popular keywords: `async/await`
...
Author also said reasons of the `async/await` syntactic in his proposal:
...
## 4. ASGI and the rise of asynchronous webservers

Before ASGI was born, WSGI was the popular standard for web applications to communicate with webservers. This standard was introduced by [PEP 333](https://peps.python.org/pep-0333/) in late 2003. It focuses on developing an interface based on the synchronous API of the network: blocking. Therefore, to implement scale performance, multi-thread is used in combination, similar to the case of Apache HTTP Server.

We have webservers implementing this standard such as [Gunicorn](https://gunicorn.org), [uWSGI](https://uwsgi-docs.readthedocs.io/en/latest/) or Nginx with the support of [ngx_http_uwsgi_module module](https://nginx.org/en/docs/http/ngx_http_uwsgi_module.html).
...
With http client, we can use [httpx](https://github.com/encode/httpx), it has a completely similar interface to [requests](https://requests.readthedocs.io/en/latest/).

For web framework, we have:
...
Python is really changing a lot in web development in general and the stack handles asynchronous - scalable application in general. Let's take a look at a few milestones before ending the article

- 1991: Python was created and supports low-level APIs for non-blocking
- 2003: WSGI was born - open the era of web development in Python
- 2005: PEP 342 introduces coroutine
- 2015: PEP 492 introduces syntaxasync/await
